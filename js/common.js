...
function ensureNavLink(href, label, iconClass = 'fas fa-link') {
  const list = document.querySelector('.navbar-links');
  if (!list || list.querySelector(`a[href="${href}"]`)) return;
  const li = document.createElement('li');
  li.innerHTML = `<a href="${href}"><i class="${iconClass}"></i> ${label}</a>`;
  const adminLi = list.querySelector('a[href="admin.html"]')?.closest('li');
  if (adminLi) list.insertBefore(li, adminLi);
  else list.appendChild(li);
}

function enhanceNavbarLinks() {
  ensureNavLink('sports.html', 'الأخبار الرياضية', 'fas fa-futbol');
  injectMemberNavButton();
}
...
async function openMemberAuthModal() {
...
function updateMemberUI() {
  ...
  if (box) {
    if (__memberSession) {
      ...
      box.innerHTML = `<strong>مرحباً ${__memberSession.full_name}</strong><br><small>${__memberSession.email}</small>`;
    } else {
